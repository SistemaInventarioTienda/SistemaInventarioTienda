...
VALUES (
    ...
    (
        3,
        'Pendiente',
        'Estado \'Pendiente\' para las ventas a crédito',
        '2025-03-26 21:30:23'
    );
...
VALUES (
    ...
    (
        7,
        'Productos',
        'Se le permite el acceso a la página de productos. Puede realizar acciones como: ver todos los productos, eliminar, agregar y modificar'
    ),
    (
        8,
        'Ventas',
...
VALUES (
        21,
        'Coca cola',
        'Esta es con un recipiente de 1.5L',
        'PROD202502190056154',
        'image_not_found.png',
        2200,
        1950,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_getAllDataFromGrpahics` (
    IN `MIN_FEC` DATE,
    IN `MAX_FEC` DATE,
...
BEGIN
	...
	-- Suma de las ventas o 0
	SELECT 
		m.Mes,
		COALESCE(SUM(
			v.MONT_SUBTOTAL + (v.MONT_SUBTOTAL * (v.PORCENT_IMPUESTO / 100)) - v.PORCENT_DESCUENTO
		), 0) AS Total
	FROM 
		meses m
	LEFT JOIN 
		tsit_venta v 
		ON DATE_FORMAT(v.FEC_VENTA, '%Y-%m') = m.Mes
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getShoppingsReport`(
	IN `MIN_FEC` DATE,
    IN `MAX_FEC` DATE
)
BEGIN
	...
	WHERE
		DATE(c.FEC_COMPRA) >= MIN_FEC AND DATE(c.FEC_COMPRA) <= MAX_FEC
	GROUP BY
