...
-- --------------------------------------------------------
CREATE TABLE IF NOT EXISTS tsit_notificaciones (
  IDENTIFICADOR_NOTIFICACION int(11) NOT NULL,
...
(10, 'Proforma', 'El usuario tiene permitido acceder al modulo de proformas');
...
(10, 10, 10, '2025-05-04 19:34:38', 1);
...
CREATE DEFINER=root@localhost PROCEDURE getTransactionReport (IN MIN_FEC_TRANSACCION DATETIME, IN MAX_FEC_TRANSACCION DATETIME)   BEGIN
    SELECT 
        ID_TRANSACCION,
        FEC_TRANSACCION,
        METODO_PAGO,
...
CREATE DEFINER=root@localhost PROCEDURE sp_cash_closing (IN MIN_FEC DATE, IN MAX_FEC DATE)   BEGIN
...
CREATE DEFINER=root@localhost PROCEDURE sp_getClientCreditReport (IN MIN_FEC DATE, IN MAX_FEC DATE)   SELECT
    cl.DSC_CEDULA,
...
CREATE DEFINER=root@localhost PROCEDURE sp_getSupplierReport ()   BEGIN
...
CREATE DEFINER=root@localhost PROCEDURE sp_products_report (IN MIN_FEC DATE, IN MAX_FEC DATE)   BEGIN
	SELECT
...
-- ----------------------------------------------------------------------------
use dbtiendasistemainventario;
-- DROP DATABASE dbtiendasistemainventario;
...
-- FALTA QUE LA NOTIFICACION TENGA UN ID AUTOINCREMENTAL Y PK