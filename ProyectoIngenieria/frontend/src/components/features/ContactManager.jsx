...
export default function ContactManager({
    ...
    const {
        ...
        handleDeleteContact,
        handleCancelEdit,
    } = useContactManager(initialContacts, onContactsChange, type);
            ...
            {!isViewMode && (
                <div className="contact-input-group">
                    <InputButton
                        ...
                        placeholder={`Ingrese el ${type === 'phone' ? 'teléfono' : 'correo'}`}
                        inputClassName="border-custom"
                        onButtonClick={(e) => handleAddOrUpdateContact(e)}
                        icon={editingIndex !== null ? Edit : Plus}
                        buttonLabel={editingIndex !== null ? "Actualizar" : "Agregar"}
                    />
                    {editingIndex !== null && (
                ...
                <div className="contacts-table">
                    <table>
                        ...
                        <tbody>
                            {contacts.map((contact, index) => (
                                <tr key={contact.id}>
                                    <td>{type === 'phone' ? contact.numeroTelefono : contact.correoElectronico}</td>
                                    {(isEditMode || isAddMode) && (
                                        <td className="action-cell">
                                            {isEditMode && (
                                                <Button
