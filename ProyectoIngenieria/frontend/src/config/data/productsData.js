export const productsData = {
    title: "Gestión de Productos",
    description: "Aprende a gestionar productos en el sistema",
    sections: [
        {
            question: "¿Cómo agregar un nuevo producto al sistema?",
            answer: [
                "Navega al módulo de 'Productos' desde la barra lateral.",
                "Haz clic en el botón 'Agregar Producto' en la esquina superior derecha.",
                "Completa el formulario con los datos requeridos: código de barras (opcional; si no lo ingresas, se generará automáticamente), nombre, descripción, categoría y subcategoría (al seleccionar una categoría, se cargarán las subcategorías disponibles).",
                "Ingresa el precio de venta y el precio de compra.",
                "Selecciona el estado del producto.",
                "Puedes agregar una imagen del producto, aunque no es obligatorio.",
                "Haz clic en 'Agregar' para crear el nuevo producto.",
                "Confirma la acción en el cuadro de diálogo que aparecerá.",
            ],
            videoUrl: "https://www.youtube.com/embed/dGmvRi8PRq8?si=MIfkBjn6lVo28zGH",
        },
        {
            question: "¿Cómo modificar la información de un producto?",
            answer: [
                "Navega al módulo de 'Productos' desde la barra lateral.",
                "Ubica el producto que deseas modificar utilizando la barra de búsqueda, los filtros por columnas o seleccionándolo directamente de la lista.",
                "Haz clic en el icono de 'Editar' (lápiz) correspondiente a ese producto.",
                "Modifica los campos necesarios en el formulario que se mostrará.",
                "Haz clic en 'Guardar cambios' para confirmar la edición.",
                "Confirma la acción en el cuadro de diálogo que aparecerá para completar el proceso.",
            ],
            videoUrl: "https://www.youtube.com/embed/rQ7F_GiGtaQ?si=WW65-w1znn5CrNPS",
        },
        {
            question: "¿Cómo eliminar un producto del sistema?",
            answer: [
                "Navega al módulo de 'Productos' desde la barra lateral.",
                "Ubica el producto que deseas eliminar utilizando la barra de búsqueda, los filtros por columnas o seleccionándolo directamente de la lista.",
                "Haz clic en el icono de 'Eliminar' (papelera) correspondiente a ese producto.",
                "Confirma la acción en el cuadro de diálogo que aparecerá haciendo clic en 'Eliminar'.",
                "El estado del producto cambiará a inactivo; sin embargo, seguirá visible en el sistema para fines de historial o referencia.",
            ],
            videoUrl: "https://www.youtube.com/embed/qw33j5t3r_A?si=B1-X43zheZD8yb6V",
        },
        {
            question: "¿Cómo buscar y filtrar productos?",
            answer: [
                "Navega al módulo de 'Productos' desde la barra lateral.",
                "Utiliza la barra de búsqueda ubicada en la parte superior para encontrar productos por nombre, código de barras o subcategoría.",
                "También puedes hacer clic en los encabezados de las columnas para ordenar los registros de forma ascendente o descendente según corresponda.",
                "Los resultados se actualizarán automáticamente mostrando únicamente los productos que coincidan con los criterios establecidos.",
            ],
            videoUrl: "",
        },
    ],
}
