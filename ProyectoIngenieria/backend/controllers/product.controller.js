...
export const getAllProducts = async (req, res) => {
  try {
    ...
    const field =
      ...
      orderByField === "DSC_DESCRIPTION" ||
      orderByField === "DSC_CODIGO_BARRAS" ||
      orderByField === "MON_VENTA" ||
      orderByField === "MON_COMPRA" ||
      orderByField === "ESTADO" ||
      orderByField === "CATEGORIA"
        ? orderByField
        : "FEC_CREATED_AT";
    ...
    const conditionOrder =
      field === "CATEGORIA"
        ? [{ model: subcategory, as: "subcategory" }, "DSC_NOMBRE", sortOrder]
        : [field, sortOrder];
...
export const registerProduct = [
  ...
  async (req, res) => {
    try {
      const {
        DSC_NOMBRE = null,
        DSC_DESCRIPTION = null,
        DSC_CODIGO_BARRAS = null,
        MON_VENTA = null,
        MON_COMPRA = null,
        SUBCATEGORIA = null,
        DSC_CODIGO_PROD = "",
      ...
      const barCode =
        DSC_CODIGO_BARRAS === null ||
        DSC_CODIGO_BARRAS === "" ||
        DSC_CODIGO_BARRAS === "undefined"
          ? createBarCode(created_at)
          : DSC_CODIGO_BARRAS;
      ...
      if (productFound) {
        ...
        return res.status(403).json({
          ...
          suggestions:
            "Si es un producto diferente cambia el código de barras, caso contrario actualiza el registro existente.",
        });
      }
...
function createBarCode(date, length = 15) {
  ...
  const randomDigits = Array.from({ length: randomLength }, () =>
