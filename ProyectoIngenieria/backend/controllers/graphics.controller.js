...
export const getAllDataFromGrpahic = async (req, res) => {
  try {
    ...
    const limit = parseInt(LIMIT_PRODUCTS) || 5;

    let current_date = FEC_CURRENT;
    if (FEC_CURRENT === "" || !FEC_CURRENT) {
    ...
    return res.json({
      ...
      top5Products: results?.[0] || [],
      products_stock_sold: results?.[1] || [],
      last12sales: results?.[2] || [],
    });
  } catch (error) {
    console.error("Error en getAllDataFromGrpahic:", error);
    return res.status(500).json({
      message: "Error interno del servidor al obtener datos de los gráficos.",
      error: error.message,
    });
  }
};
