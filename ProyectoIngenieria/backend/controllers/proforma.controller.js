...
import {
  ...
  isDateValid,
  isProductsValid,
} from "../logic/proforma/proforma.logic.js";
...
export const createProForma = async (req, res) => {
  ...
  try {
    ...
    if (!isDateValid(currentDate, FEC_LIMITE))
      return res.status(400).json({
        message:
          "La fecha limite no es valida. Favor ingresar una mayor o igual a la fecha actual.",
      });

    const validateDetails = await isProductsValid(details_list);
    if (!validateDetails.success)
      return res.status(400).json({ message: validateDetails });
...
export const getAllProForma = async (req, res) => {
  try {
    const {
      page = 1,
      pageSize = 5,
      orderByField = "DSC_CODIGO_BARRAS",
      order = "asc",
    } = req.query;
    ...
    const field =
      ...
      orderByField === "FEC_CREACION" ||
      orderByField === "FEC_LIMITE" ||
      orderByField === "DSC_CODIGO_BARRAS" ||
      orderByField === "ESTADO"
        ? orderByField
        : "DSC_CODIGO_BARRAS";
    ...
    if (rows.length === 0)
      return res
        .status(204)
        .json({ message: "No se encontraron facturas proformas." });
...
export const searchProForma = async (req, res) => {
  try {
    ...
    // Sequelize devuelve el result set como objeto indexado
    ...
    // YA NO se hace JSON.parse
    const parsedResults = rows.map((r) => ({
      ...r,
      PRODUCTS_LISTS: Array.isArray(r.PRODUCTS_LISTS) ? r.PRODUCTS_LISTS : [], // por seguridad si viene NULL
    }));
...
export const deleteProForma = async (req, res) => {
  try {
    ...
    const proformaFound = await Proforma.findOne({
      where: { ID_PROFORMA: id },
    });
    if (!proformaFound)
      return res
        .status(404)
        .json({ message: "Factura proforma no encontrada." });
    ...
    res
      .status(200)
      .json({ message: "Factura proforma anulada correctamente." });
  } catch (error) {
    res.status(500).json({
      message: "Error al anular la factura proforma.",
      error: error.message,
    });
  }
};
...
export const proformaSale = async (req, res) => {
  try {
    ...
    // Sequelize devuelve un objeto indexado
    const rows = Object.values(results);
    const count = rows.length;
    ...
    // PRODUCTS_LISTS ya es un array JS (o null)
    const parsedResults = rows.map((r) => ({
      ...r,
      PRODUCTS_LISTS: Array.isArray(r.PRODUCTS_LISTS) ? r.PRODUCTS_LISTS : [],
    }));

    res.status(200).json({
      message: "Proforma encontrada correctamente.",
      proforma: parsedResults,
    });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
