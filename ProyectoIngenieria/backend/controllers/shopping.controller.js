...
export const searchShopping = async (req, res) => {
  try {
    ...
    const field =
      orderByField === "FEC_COMPRA" ||
      orderByField === "FEC_ENTRADA" ||
      orderByField === "MON_TOTAL" ||
      orderByField === "DSC_METODO_PAGO" ||
      orderByField === "PROVEEDOR" ||
      orderByField === "PRODUCTO" ||
      orderByField === "ESTADO"
        ? orderByField
        : "FEC_CREATED_AT";
  ...
  try {
    ...
    for (const product of PRODUCTS_LIST) {
      try {
        ...
        if (detailsSaved) {
          sumTotal =
            sumTotal + product.MON_PRECIO_COMPRA * product.MON_CANTIDAD;
          ...
          if (productFound) {
            productFound.CANTIDAD =
              productFound.CANTIDAD + product.MON_CANTIDAD;
            productFound.save();
          }
        }
      } catch (err) {
        detailsShoppingErrors.push(
          `Error al agregar el producto con código de barras: ${product.DSC_CODIGO_BARRAS}`
        );
      }
    }
    ...
    return res.status(200).json({
      ...
      errors: detailsShoppingErrors ? detailsShoppingErrors : null,
    });
  } catch (error) {
...
export const deleteShopping = async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(404)
        .json({ message: "El usuario no tiene permiso de anular la compra." });
    }
    ...
    if (!EightDaysHavePassed(shopping.FEC_COMPRA)) {
      return res
        .status(404)
        .json({ message: "El tiempo para anular la venta ha expirado." });
    }
...
function validateregister(elements) {
  for (const key in elements) {
    if (!elements[key]) {
      return {
        status: 400,
        user_message: `El campo ${key} no puede estar vacio.`,
      };
    }
  }
  return true;
...
const clearShoppingDetails = (data) => {
  ...
  for (const key in data) {
    ...
    // PRODUCTS_LISTS ya viene como array (JSON_ARRAYAGG)
    if (!Array.isArray(shopping.PRODUCTS_LISTS)) {
      shopping.PRODUCTS_LISTS = [];
    }

    // Normalizar cada producto por seguridad
    shopping.PRODUCTS_LISTS = shopping.PRODUCTS_LISTS.map((product) => ({
      DSC_NOMBRE: product.DSC_NOMBRE ?? "",
      DSC_CODIGO_BARRAS: product.DSC_CODIGO_BARRAS ?? "",
      MON_CANTIDAD: Number(product.MON_CANTIDAD) || 0,
      MON_PRECIO_COMPRA: Number(product.MON_PRECIO_COMPRA) || 0,
    }));

    // DETALLE_PRODUCTO ahora es string plano
    if (typeof shopping.DETALLE_PRODUCTO !== "string") {
      shopping.DETALLE_PRODUCTO = "";
    }

    // Normalizar tipos básicos
    shopping.ESTADO = Number(shopping.ESTADO) || 0;
    shopping.MON_TOTAL = Number(shopping.MON_TOTAL) || 0;
  }
